    ...
    def _apt_update(self):
        ...
        if self.flags.get("allow_sudo") == False:
            try:
                ...
                log.warning(
                    f"This command requires sudo. Please ensure the `allow_sudo` flag is set to True."
                )
            except subprocess.CalledProcessError as e:
                log.warning(
                    f"This command requires the `allow_sudo` flag to be on in `config.yaml`.\n{e}"
                )
                return
        ...
        if self.flags.get("allow_sudo") == False:
            for pkg in self.packages:
                ...
                try:
                    ...
                    log.warning(
                        f"This command requires `sudo`. Please verify `allow_sudo` flag is TRUE in `config.yaml`.\n{e}"
                    )
                    failed.append(pkg)
...
class CargoInstaller:
    ...
    def _check_current_installs(self):
        ...
        currently_installed_raw = subprocess.run(
        ...
        for package in self.packages:
            if package in currently_installed_packages:
                ...
                search_results = subprocess.run(
                    ["cargo", "search", f"{package}"], capture_output=True, text=True
                )
...
class InstallManager:
    def __init__(self, user_config: dict, install_config: dict):
        ...
        self.apt_list = (
            install_config.get("install_settings", {})
            .get("installed_by", {})
            .get("apt", [])
        )
        self.cargo_list = (
